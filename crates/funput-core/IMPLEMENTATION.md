...
## Phase 9 — Telex (sau VNI)

**File:** `input_method/telex.rs`

Telex dùng **cùng** `unicode/`, `validation/`, `composition/transform.rs`. Chỉ thêm mapping key:
...
1. Telex key → map sang action nội bộ (cùng enum với VNI tone/shape/stroke)
2. Reuse toàn bộ pipeline Phase 1–7
3. Test parity: mỗi case VNI có case Telex tương ứng

```rust
// tests/telex_parity.rs
assert_eq!(apply("a", 's', Telex), apply("a", '1', Vni)); // both → á
```

### Done khi

Bảng parity VNI ↔ Telex pass; `InputMethod::Telex` production-ready.
